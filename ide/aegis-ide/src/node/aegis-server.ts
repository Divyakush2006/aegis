/**
 * Backend service: drives the Aegis language server from Theia's Node process.
 *
...
const FINDINGS_NOTIFICATION = 'aegis/findings';
const STATUS_NOTIFICATION = 'aegis/status';
...
export function findCompilerRoot(
    explicit: string | undefined = process.env.AEGIS_COMPILER_ROOT,
    ...
    for (const start of starts) {
        ...
        while (true) {
            ...
            if (fs.existsSync(path.join(candidate, 'src', 'aegis', '__init__.py'))) {
                return candidate;
            }
...
@injectable()
export class AegisServerImpl implements AegisService {
    @inject(ILogger) protected readonly logger: ILogger;

    protected client: AegisClient | undefined;
    ...
    protected async start(): Promise<MessageConnection> {
        this.logger.info(`Starting the Aegis language server: ${PYTHON} in ${COMPILER_ROOT}`);
        const child = spawn(PYTHON, ['-m', 'aegis.server.lsp_server'], {
            cwd: COMPILER_ROOT,
            env: {
                ...process.env,
        ...
        child.stderr?.on('data', data => this.logger.debug(`[aegis] ${data}`));
        child.on('exit', code => {
            this.logger.warn(`Aegis language server exited with code ${code}`);
            this.connection = undefined;
            this.process = undefined;
        });
        child.on('error', error => {
            this.logger.error(`Failed to start the Aegis language server: ${error}`);
            this.connection = undefined;
            this.process = undefined;
        });
        ...
        connection.onNotification(FINDINGS_NOTIFICATION, (result: AegisAuditResult) =>
        ...
        this.logger.info('Aegis language server ready');
        return connection;
    }
    ...
    protected async execute<T>(command: string, args: unknown[], fallback: T): Promise<T> {
        try {
            ...
            this.logger.error(`Aegis command ${command} failed: ${error}`);
            return fallback;
        }
    }

    async audit(uri: string): Promise<AegisAuditResult> {
        return this.execute<AegisAuditResult>('aegis.audit', [uri], {
            uri,
            revision: '',
            stats: {},
            findings: [],
            exclusions: [],
            error: 'the Aegis language server is unavailable'
        });
    }

    async explain(uri: string, functionName: string): Promise<AegisExplanation> {
        return this.execute<AegisExplanation>('aegis.explain', [uri, functionName], {
    ...
    async adjudicate(uri: string): Promise<AegisAuditResult> {
        return this.execute<AegisAuditResult>('aegis.adjudicate', [uri], {
            uri,
            revision: '',
            stats: {},
            findings: [],
    ...
    async aiStatus(): Promise<AegisAiStatus> {
        return this.execute<AegisAiStatus>('aegis.aiStatus', [], {
            configured: false,
            error: 'the Aegis language server is unavailable'
        });
    }
    ...
    async buildLlvm(uri: string): Promise<{ uri: string; llvm?: string; error?: string }> {
        return this.execute('aegis.build', [uri], {
            uri,
            error: 'the Aegis language server is unavailable'
        });
    }
