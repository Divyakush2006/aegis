...
This is what makes Aegis a compiler rather than an analyser that happens to
...
from ..diagnostics import AegisError
...
def ensure_llvm() -> None:
    ...
    if not LLVM_AVAILABLE:
        raise AegisError(
            "LLVM code generation requires llvmlite. Install it with: pip install 'aegis-compiler[codegen]'"
        )
    if not _initialised:
...
def _llvm_type(name: str):
    """Map an Aegis type string onto an LLVM type."""
...
class FunctionEmitter:
    """Lowers one :class:`~aegis.ir.instructions.Function` into an LLVM function."""
...
class LLVMEmitter:
    """Lowers a whole :class:`~aegis.ir.instructions.Module` to LLVM IR."""

    def __init__(self, module: I.Module, name: str = "aegis") -> None:
        ensure_llvm()
        self.source = module
...
def emit_ir(module: I.Module, name: str = "aegis") -> str:
