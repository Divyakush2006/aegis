"""AegisAST -- the project's own abstract syntax tree.

pycparser supplies ``c_ast``; nothing downstream of this module is allowed to
see it. Every later phase consumes AegisAST, so if the reused parser is ever
replaced by a hand-written one, only :mod:`aegis.frontend.adapter` changes.
