...
The custom ``prahari/findings`` notification carries full path traces to the
...
FINDINGS_NOTIFICATION = "prahari/findings"
#: Custom notification for audit progress.
STATUS_NOTIFICATION = "prahari/status"

AUDIT_COMMAND = "prahari.audit"
BUILD_COMMAND = "prahari.build"
...
class PrahariLanguageServer(LanguageServer):
...
def _publish(ls: PrahariLanguageServer, uri: str, index: SemanticIndex | None) -> None:
...
def did_open(ls: PrahariLanguageServer, params: lsp.DidOpenTextDocumentParams) -> None:
    _publish(ls, params.text_document.uri, ls.compile(params.text_document.uri))


@server.feature(lsp.TEXT_DOCUMENT_DID_SAVE)
def did_save(ls: PrahariLanguageServer, params: lsp.DidSaveTextDocumentParams) -> None:
    _publish(ls, params.text_document.uri, ls.compile(params.text_document.uri))


@server.feature(lsp.TEXT_DOCUMENT_DID_CLOSE)
def did_close(ls: PrahariLanguageServer, params: lsp.DidCloseTextDocumentParams) -> None:
...
def completion(
    ls: PrahariLanguageServer, params: lsp.CompletionParams
) -> lsp.CompletionList:
...
def hover(ls: PrahariLanguageServer, params: lsp.HoverParams) -> lsp.Hover | None:
    index = ls.index_for(params.text_document.uri)
    if index is None or index.program is None:
        return None
...
def _word_at(ls: PrahariLanguageServer, params) -> str:
    try:
...
def audit(ls: PrahariLanguageServer, *args: Any) -> dict:
...
def explain(ls: PrahariLanguageServer, *args: Any) -> dict:
    ...
    if index is None or name is None:
        return {"error": "usage: prahari.explain <uri> <function>"}
...
def adjudicate(ls: PrahariLanguageServer, *args: Any) -> dict:
    """Audit, then review each finding through the configured model.

    Separate from ``prahari.audit`` on purpose. An audit is free, local and runs
...
def ai_status(ls: PrahariLanguageServer, *args: Any) -> dict:
...
def build(ls: PrahariLanguageServer, *args: Any) -> dict:
