...
class HttpGateway:
    ...
    def complete(
        ...
        if over_budget:
            raise GatewayError(
                ...
                "raise PRAHARI_AI_MAX_REQUESTS to continue"
            )
    ...
    def hint(self, detail: str) -> str:
        ...
        if "agentic harness" in lowered:
            return (
                ...
                "choose another free model (prahari ai --models)"
            )
    ...
    def status_error(self, status: int, raw: bytes) -> GatewayError:
        ...
        if status == 401:
            message = "the API key was rejected (401); check OPENROUTER_API_KEY / PRAHARI_API_KEY in .env"
        elif status == 402:
