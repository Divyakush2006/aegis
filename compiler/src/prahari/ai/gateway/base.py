...
Two implementations satisfy it. :class:`~prahari.ai.gateway.null.NullGateway`
is deterministic and needs no network; it is what CI and the evaluation control
row run on. :class:`~prahari.ai.gateway.anthropic.AnthropicGateway` calls a real
model. Selecting between them is :func:`~prahari.ai.gateway.build_gateway`'s job
