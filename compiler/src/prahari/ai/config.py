...
Resolved from three places, lowest precedence first: a committed ``prahari.toml``,
...
**Models are chosen per use case, not globally.** Prahari consults a model in two
...
request is built, whether it came from ``.env``, ``prahari.toml`` or a command
...
#: ``PRAHARI_API_KEY`` is checked first so a user can point Prahari at a different
#: key from the one their other tooling uses.
KEY_VARIABLES = ("PRAHARI_API_KEY", "OPENROUTER_API_KEY", "ANTHROPIC_API_KEY")
...
CONFIG_NAMES = ("prahari.toml", ".prahari.toml")

#: Secrets file names searched upward from the working directory. Unlike
#: ``prahari.toml`` this one *may* carry a credential: it is git-ignored, which
...
class AIConfig:
    ...
    def resolved_cache_dir(self) -> Path:
        ...
        return base / "prahari" / "adjudication"
...
def _read_toml(path: Path) -> dict:
    ...
    try:
        ...
        return {}
    section = data.get("tool", {}).get("prahari", {}).get("ai", {})
...
def load_config(
    ...
    """Resolve configuration from ``prahari.toml``, then ``.env``, then the environment.

    The ordering is what deployments expect. ``prahari.toml`` holds the choices a
    ...
    # PRAHARI_NO_DOTENV exists for the test suite: a developer's real key in the
    # project's .env must never turn a test into an API call.
    skip_dotenv = _to_bool(env.get("PRAHARI_NO_DOTENV"), False)
    ...
    if settings.get("PRAHARI_AI_PROVIDER"):
        config.provider = settings["PRAHARI_AI_PROVIDER"].strip()
        config.sources["provider"] = origin.get("PRAHARI_AI_PROVIDER", "PRAHARI_AI_PROVIDER")
    ...
    for field_name, value in defaults.items():
        ...
        # prahari.toml or the environment always wins.
    ...
    take("PRAHARI_MODEL", "model")
    take("PRAHARI_MODEL_INTERACTIVE", "interactive_model")
    take("PRAHARI_MODEL_FALLBACKS", "fallback_models", _to_models)
    take("PRAHARI_API_BASE", "base_url")
    take("PRAHARI_AI_FREE_ONLY", "free_only", _to_bool)
    take("PRAHARI_AI_REASONING", "reasoning_effort", _to_effort)
    take("PRAHARI_AI_REASONING_INTERACTIVE", "interactive_reasoning_effort", _to_effort)
    take("PRAHARI_AI_RPM", "requests_per_minute", _to_float)
    take("PRAHARI_AI_CACHE", "cache_enabled", _to_bool)
    take("PRAHARI_AI_TIMEOUT", "timeout", _to_float)
    take("PRAHARI_AI_RETRIES", "max_retries", _to_int)
    take("PRAHARI_AI_MAX_REQUESTS", "max_requests", _to_int)
    take("PRAHARI_AI_MAX_TOKENS", "max_tokens", _to_int)
    take("PRAHARI_AI_REQUIRE_TOOLS", "require_tools", _to_bool)
    if settings.get("PRAHARI_AI_CACHE_DIR"):
        config.cache_dir = Path(settings["PRAHARI_AI_CACHE_DIR"])
        config.sources["cache_dir"] = origin.get("PRAHARI_AI_CACHE_DIR", "PRAHARI_AI_CACHE_DIR")
