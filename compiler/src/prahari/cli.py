...
its own -- ``prahari ir`` dumps three-address code, ``prahari cfg`` dumps the graph,
``prahari dataflow`` dumps the classical analyses, ``prahari audit`` runs everything.
...
def _expand(paths: list[str]) -> list[str]:
    ...
    for raw in paths:
        ...
        if path.is_dir():
            ...
            print(f"prahari: no such file or directory: {raw}", file=sys.stderr)
    return out
...
def _load(args) -> object:
    ...
    if not files:
        print("prahari: no input files", file=sys.stderr)
        raise SystemExit(2)
    return build_index(
        files,
        specs=SpecTable(),
...
def _adjudicate(index, args) -> None:
    ...
    if not adjudicator.enabled:
        ...
        print(
            f"prahari: no model configured -- {reason}; static verdicts retained{hint}",
            file=sys.stderr,
        )
        return
    print(
        f"prahari: adjudicated {stats.adjudicated}/{stats.candidates} findings "
    ...
    for failure in stats.failures[:3]:
        print(f"prahari:   {failure}", file=sys.stderr)
...
def _report_routing(gateway) -> None:
    ...
    if len(routed) > 1 or (routed and next(iter(routed)) != getattr(inner, "model_id", "")):
        ...
        print(f"prahari:   answered by: {answered}", file=sys.stderr)
    limit = getattr(inner, "rate_limit", {}) or {}
    if limit.get("remaining") is not None:
        print(
            f"prahari:   rate limit: {limit['remaining']} of {limit.get('limit', '?')} remaining",
            file=sys.stderr,
        )
...
def _list_models(config) -> int:
    ...
    try:
        ...
        print(
            f"prahari: {config.provider} does not publish a model catalogue; "
            "set PRAHARI_MODEL directly",
            file=sys.stderr,
        )
        return 2
    except RuntimeError as exc:
        print(f"prahari: {exc}", file=sys.stderr)
        return 1
...
def cmd_ai(args) -> int:
    ...
    if not config.configured:
        print(
            ...
            "project root\n(or export PRAHARI_API_KEY / ANTHROPIC_API_KEY). The compiler and "
...
def cmd_build(args) -> int:
    ...
    if index.errors() and not args.force:
        ...
        print("prahari: refusing to generate code for a program with errors "
    ...
    try:
        ...
        print(f"prahari: code generation failed: {exc}", file=sys.stderr)
        return 2
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"prahari {VERSION}")
